...
describe('profileReducer', () => {
  test('Sets name to new name when SET_NAME action is received', () => {
    const fakeAction = { type: ActionTypes.SET_NAME, payload: { name: 'Haichen Wang' } };
    const originalState = {
        name: 'Sergio Diaz',
  ...
  test('Sets bio to new bio when SET_BIO action is received', () => {
    const fakeAction = { type: ActionTypes.SET_BIO, payload: { bio: 'This is my new bio!' } };
    const originalState = {
        name: 'Sergio Diaz',
        bio: 'Junior in the college of engineering. Selling a bunch of textbooks and clothes I don\'t need.'
    }
  ...
  test('Sets name and bio to new values when SET_NAME and SET_BIO action is received', () => {
    const fakeAction1 = { type: ActionTypes.SET_NAME, payload: { name: 'Haichen Wang' } };
    const fakeAction2 = { type: ActionTypes.SET_BIO, payload: { bio: 'This is my new bio!' } };
    const originalState = {
        name: 'Sergio Diaz',
        bio: 'Junior in the college of engineering. Selling a bunch of textbooks and clothes I don\'t need.'
    }
    ...
    const actualState = profileReducer(profileReducer(originalState, fakeAction1), fakeAction2);
    expect(actualState).toEqual(expectedState);
  })
});
